...
def MyBids(permissions,name,UserID): # creates mybids page and displayes query results
        mybids = tkinter.Tk()
        mybids.title('mybids')
        mybids.configure(background ='light blue')
        mybids.geometry("1080x700")
        title_text = str("Welcome to mybids " + name)
        title = tkinter.Label(mybids,text=(title_text)).pack(side=tkinter.TOP)
        ID = str(UserID)
        result = executequery("SELECT Bids.*,Event.EventName FROM Bids WHERE Bids.EventID=Event.EventID AND Bids.UserID ="+ID,None,False)
        for bids in result:
                BidEvent=bids[4]
                state = bids[3]
                if State == 1:
                    ...
                    State = "unapproved"
                bidtext = "Bid: Event: " +BidEvent+" | Selected: "+State
                bid = tkinter.Label (mybids, text=bidtext).pack()
        backtomenu=tkinter.Button(mybids,text='Back to Menu',command=lambda:[mybids.destroy(),Menu(permissions,name,UserID)])
